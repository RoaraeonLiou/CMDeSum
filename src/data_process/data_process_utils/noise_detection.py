...
def if_only_param_comment(comment, language):
    only_param_comment = None
    if language == 'java':
        only_param_comment = re.compile(r'^(\s*@)')
    elif language == 'python':
        only_param_comment = re.compile(r'^(\s*:\s*param\s+)')
    if only_param_comment.match(comment):
        return True
    else:
        return False
...
def cleanCommentTag(comment):
    ...
    clean_p1 = re.compile(r'([\s/*=-]+)$|^([\s#/*=-]+)')
    clean_p2 = re.compile(r'^([\s*]+)')
...
def get_first_sentence(comment):
...
def if_Partial(raw_comment, benchmark_comment):
    correct_comment = get_first_sentence(raw_comment)
...
def if_EmptyFunc(raw_code, language):
    pattern = None
    if language == 'java':
        pattern = re.compile(r'\([\w\W]*\)[\s\S]*{\s*}$')
    elif language == 'python':
        pattern = re.compile(r'^(def\s+[\w\d_]+)(\([\w\W]*\):\s*pass\s*)$')
...
def if_CommentedOut(raw_code, language='java'):
...
if __name__ == '__main__':
    ...
    print(raw_comment)
    print(get_first_sentence(raw_comment))
    ...
    comment = get_first_sentence(raw_comment)
    print(comment)
